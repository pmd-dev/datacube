 ...
 * the cutover to the new ID mapping creation method is backwards compatible.
 */
public class OldHBaseIdService implements IdService {
    ...
    public OldHBaseIdService(Configuration configuration, byte[] lookupTable,
    ...
    public Optional<byte[]> getId(int dimensionNum, byte[] input, int numIdBytes) throws IOException, InterruptedException {
        // This version didn't support these semantics, instead getId had the semantics getOrCreateId has now.
...
//            log.debug("Allocated new id " + id);
        ...
        if (swapSuccess) {
            ...
            // Recurse, try again.
            return getOrCreateId(dimensionNum, input, numIdBytes);
        }
    }
...
//                        log.debug("New value, dimension=" + dimensionNum + " id=" + id);
    ...
    public byte[] makeCounterKey(int dimensionNum) {
